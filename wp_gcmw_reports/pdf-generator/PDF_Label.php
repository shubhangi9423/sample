...
class PDF_Label extends FPDF {
	...
	// Print a label
	function Add_Label($name,$add,$currSection) {
		$this->_COUNTX++;
		...
		if(!empty($currSection) ){
			...
			$this->SetFillColor(181, 173, 173);
			$this->MultiCell($this->_Width - $this->_Padding, $this->_Line_Height*1, $currSection,0,'L',true);
		 }
		$this->SetXY($_PosX, $_PosY+($this->_Line_Height*1.5));
		$this->SetFont(Arial,B,10);	
		$this->MultiCell($this->_Width - $this->_Padding, $this->_Line_Height, $name, 0, 'L');
		$this->SetXY($_PosX, $_PosY+($this->_Line_Height*1.5));
		$this->SetFont(Arial,'',10);
		$this->MultiCell($this->_Width - $this->_Padding, $this->_Line_Height, $add, 0, 'L');
		$this->SetLineWidth(0.1);
		$this->SetDash(1,1); //1mm on, 1mm off
		$_PosY = $this->_Margin_Top + ($this->_COUNTY+1)*($this->_Height+$this->_Y_Space) + $this->_Padding;
        $_PosX_start = $this->_Margin_Left;
		$_PosX_end = $this->_Margin_Left + 2 * ($this->_Width+$this->_X_Space) + $this->_Padding;
		write_log('Line '.$_PosX.', '.$_PosY);
		$this->Line($_PosX_start,$_PosY,$_PosX_end,$_PosY);
		$_PosX = $this->_Margin_Left + 1 *($this->_Width+$this->_X_Space);
        $_PosY_start = $this->_Margin_Top + $this->_COUNTY*($this->_Height+$this->_Y_Space) + $this->_Padding;
		$_PosY_end =  $this->_Margin_Top + ($this->_COUNTY+1)*($this->_Height+$this->_Y_Space) + $this->_Padding;
		$this->Line($_PosX,$_PosY_start,$_PosX,$_PosY_end);
